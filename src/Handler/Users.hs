...
import Model
    ...
    , UserId
    , User
      ( User, userName, userEmail, userAdmin
      )
    , UserPhoto (UserPhoto)
    , EntityField
      ( UserPhotoUser, UserId, UserPhotoAttribution, UserEmail, UserPhotoPhoto
      , UserPhotoMime, UserName, UserPassword, UserAdmin
      )
    )
...
postUserDeleR uid = do
    ...
    case fr of
      FormSuccess () -> do
          ...
          case user of
            Just (Entity _ (User _ _ _ True _)) -> do
...
postUserR uid = do
    ...
    case fr of
      FormSuccess (User email _ name _ admin,(Just fi,attrib)) -> do
      ...
      FormSuccess (User email _ name _ admin,(Nothing,attrib)) -> do
          void $ runDB $ update $ \x -> do
            set x [ UserEmail =. val email
                  , UserName =. val name
                  , UserAdmin =. val admin
